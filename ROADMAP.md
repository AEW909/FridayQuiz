...
**Tests/verification:** `npm run typecheck`, `npm run test:standings` (7/7), and `npm run build` passed. The test suite covers ties, corrected momentum, missing-versus-zero result state, class membership rejection, post-submission roster-count locking, and dedicated school-admin group recognition.

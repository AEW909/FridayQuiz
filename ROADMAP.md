...
**Tests/verification:** `npm run typecheck`, `npm run test:standings` (9/9), and `npm run build` passed. The test suite covers ties, corrected momentum, missing-versus-zero result state, class membership rejection, post-submission roster-count locking, retirement minimums, and protection against retired-team resurrection during a later roster edit.
